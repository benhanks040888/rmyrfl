...
@section('content')
  ...
  <a href="{{URL::route('admin.client.add',array($category))}}" class="btn badge btn-info">+ Add New Data</a>
	...
	<script>	
		...
		$(document).ready(function(){
			...
			var oTable = $(".datatable").dataTable({
				...
				"sAjaxSource" : "{{URL::route('admin.client.list',array($category))}}",
				'iDisplayLength' : 25,
				"aaSorting": [[ 1, "asc" ]],
				'aafilter':'yes',
				"aoColumns":[
				...
				"fnDrawCallback": function ( oSettings ) {
					...
					for ( var i=0, iLen=oSettings.aiDisplay.length ; i<iLen ; i++ ){		
						...
						$('td:eq(<?php echo $num_fields - $hidden_field ?>)', oSettings.aoData[ oSettings.aiDisplay[i] ].nTr ).html("<table><tr><td><a href=\"{{URL::Route('admin.client.edit',array($category,'id' => ''))}}/"+oSettings.aoData[i]._aData[0]+"\" class=\"btn btn-primary btn-xs\">EDIT</a></td>"
		...
		function deleteRow(id){
			if(confirm("Are you sure you want to delete this row?")){
				$.ajax({
					url: "{{URL::route('admin.client.delete')}}",  //Server script to process data
					type: 'POST',
					dataType: 'json',
					data: "id="+id,
					cache: false,
		...
		function openView(id){
			$.ajax({
					url: "{{URL::route('admin.client.detail')}}",  //Server script to process data
					type: 'POST',
					dataType: 'json',
					data: "id="+id,
					cache: false,
