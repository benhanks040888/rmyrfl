...
class SpeakerController extends BaseSiteController {
	...
	public function getIndex($lang = 'id') {
		...
		$customer = GeneralInfo::Key('customer-speaker-summary')->first();
	...
	public function getSesiGroup($lang = 'id') {
		...
		$content = GeneralInfo::Key('speaker-group')->first();
		if ($content) {
			$data['content'] = $content->value_id;
			if ($lang == 'en') {
				$data['content'] = $content->value_en;
		...
		$data['pageTitle'] = "Corporate Speaker - " . $data['title'];
		$data['pageDescription'] = "Corporate Speaker - " . $data['title'];
		return View::make('pages.free', $data);
	}

	public function getProduct($lang = 'id') {
		$data['title'] = trans('menu.menu-product');
		$content = GeneralInfo::Key('speaker-product')->first();
		if ($content) {
			$data['content'] = $content->value_id;
			if ($lang == 'en') {
				$data['content'] = $content->value_en;
	...
	public function getFreeGift($lang = 'id') {
		...
		$content = GeneralInfo::Key('speaker-gift')->first();
		if ($content) {
			$data['content'] = $content->value_id;
			if ($lang == 'en') {
				$data['content'] = $content->value_en;
	...
	public function getAssociate($lang = 'id') {
		...
		$content = GeneralInfo::Key('speaker-association')->first();
		if ($content) {
			$data['content'] = $content->value_id;
			if ($lang == 'en') {
				$data['content'] = $content->value_en;
